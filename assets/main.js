...
jQuery(function ($) {
  ...
  $subNavTrigger.each(function (ev) {
    ...
    var openClass = 'sub-nav--open'; // attach a corresponding data attribute for later use on the click handler

    $subnav.data('sub-nav-pane', $this.data('sub-nav-link'));
    $this.on('click', function (e) {
      e.preventDefault(); // is the corresponding subnav currently open *before* any changes occur?

      var isOpen = $subnav.hasClass(openClass); // remove all open subnav classes, therefore closing any that are open

      $('.' + openClass).removeClass(openClass); // if the corresponding subnav was closed already, user intends to open it
